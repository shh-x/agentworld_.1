...
const LLM_CONFIG = {
  ...
  model: 'llama3',
  maxTokens: 25,         // ⬇ was 150 — forces short replies
  temperature: 1.0,
}
  ...
  try {
    const res = await fetch(LLM_CONFIG.ollamaUrl, {
      ...
      body: JSON.stringify({
        ...
        prompt: prompt,
        stream: false,
        options: { num_predict: LLM_CONFIG.maxTokens, temperature: LLM_CONFIG.temperature }
      })
    })
...
function buildAgentPrompt(agent, context, transcript) {
  ...
  const memories = agent.memory
    .slice(0, 4)
    .map(m => {
      const ago = WORLD.gameTick - m.tick
      return `- ${m.detail} (${ago} seconds ago)`
    })
    .join('\n') || '- nothing witnessed'
  ...
  if (agent.role === 'impostor') {
    ...
    if (accuserNames) {
      ...
      Do NOT admit anything. Sound helpful. All lowercase.`
    }
  } else {
    ...
    if (blunder) {
      ...
      Sound unsure. Only reference your WITNESSED EVENTS. All lowercase.`
    } else {
      ...
      Only reference YOUR WITNESSED EVENTS. Do NOT invent locations or names. All lowercase.`
    }
  }

  // ── Sentence starter style injection ──
  const starters = {
    paranoid:   'Start with a question accusation.',
    analytical: 'Start with a location fact.',
    aggressive: 'Start with a name directly.',
    passive:    'One or two words only.',
    social:     'Start with "who" or "anyone".',
    deceptive:  'Start with "i was" or "literally".'
  }
  const turnStarter = starters[p.type] || 'One or two words only.'
...
window.AGENT_ROSTER = AGENT_ROSTER
const PERSONALITIES = {
...
class WorldState {
  ...
  processTasks() {
    for (const agent of this.agents.filter(a => a.alive && !a.isPlayer)) {
  ...
  async tick() {
    ...
    this.agents.filter(a => a.alive && !a.isPlayer).forEach(a => this.moveAgent(a))
    this.agents
      .filter(a => a.alive && a.isPlayer && a.path && a.path.length > 0)
      .forEach(a => this.moveAgent(a))
    this.processTasks()
    this.processKills()
